/**
 * JavaScript para gesti�n de Favoritos
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.fullday-favorite-btn', function(e) {
        ...
        if (!postId) {
            console.error('No se encontr� el ID del post');
            return;
        }
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    if (response.data.is_favorite) {
                        ...
                        // Si estamos en la p�gina de favoritos, remover la tarjeta con animaci�n
                        if ($('.favoritos-container').length) {
                            const $card = $btn.closest('.favorito-card');
                            $card.fadeOut(300, function() {
                                $(this).remove();

                                // Si ya no hay m�s tarjetas, mostrar estado vac�o
                    ...
                    if (response.data && response.data.require_login) {
                        if (typeof window.openFavoriteLoginPopup === 'function') {
                            ...
                            // Fallback: redirigir a la p�gina de login
    ...
    function showMessage(message, type) {
        // Buscar contenedor de mensajes en favoritos o crear uno gen�rico
        let $messageContainer = $('#favoritos-message');

        if (!$messageContainer.length) {
            // Si no existe, buscar uno gen�rico o crearlo temporalmente
    ...
    /**
     * Mostrar estado vac�o en favoritos
     */
    function showEmptyState() {
        const emptyStateHTML = `
            <div class="empty-state">
                ...
                <p>Explora nuestras experiencias y guarda tus favoritas para verlas m�s tarde</p>
